...
- Prevented stale async translation responses from replacing newer results after text, language, or retry changes
- Fixed a Google Translate dictionary parsing panic when the response contains incomplete dictionary groups
- Kept edited OCR recognized text in sync with copy and translate actions

### Changed

- Moved translation work onto the shared async runtime handle instead of blocking inside the UI command path
- Scoped service card button sizing to action buttons so enable switches keep their native layout
